...
version line. Service-side only (keyless reads now attribute to the unified "pythia-self" identity
regardless of self-connector state; see `CHANGELOG.md` at the repo root) — no changes to this package's
...
version line. Service-side only (per-consumer transaction attribution in the Pyth ledger + `/pyth`
`byConsumer`, and a live Activity "pulse"; see `CHANGELOG.md` at the repo root) — no changes to this
package's own source, API surface, or behavior.

## 2.7.29 — 2026-08-05

Version alignment: the client jumps from `2.7.28` to `2.7.29` to align with the unified Pythia service
version line. Service-side only (the Connectors list gained a per-row copy button for the dual API link
key; see `CHANGELOG.md` at the repo root) — no changes to this package's own source, API surface, or
behavior.

## 2.7.28 — 2026-08-04

Version alignment: the client jumps from `2.7.27` to `2.7.28` to align with the unified Pythia service
version line. Service-side only (the Activity tab's on-chain read no longer discards the ledger total
when a gap day has no row; see `CHANGELOG.md` at the repo root) — no changes to this package's own
...
version line. Service-side only (petitions again count only client-served reads, not the automaton's own
dirty reads; transactions still counted — see `CHANGELOG.md` at the repo root) — no changes to this
package's own source, API surface, or behavior.

## 2.7.24 — 2026-08-04

Version alignment: the client jumps from `2.7.23` to `2.7.24` to align with the unified Pythia service
version line. Service-side only (Pythia's own automaton on-chain fires now count in her Pyth ledger —
they previously bypassed the meter; see `CHANGELOG.md` at the repo root) — no changes to this package's
own source, API surface, or behavior.
...
version line. Service-side only (Pythia adopted khronoton-core 0.7.0 and registered its event-driven
resolver as `evented` for native scheduleless handling + a resolver roster; see `CHANGELOG.md` at the
repo root) — no changes to this package's own source, API surface, or behavior.

## 2.7.21 — 2026-08-03

Version alignment: the client jumps from `2.7.20` to `2.7.21` to align with the unified Pythia service
version line. Service-side only (a pre-existing evented cronoton is migrated off its stale schedule at
boot, plus an override delete for system cronotons; see `CHANGELOG.md` at the repo root) — no changes to
